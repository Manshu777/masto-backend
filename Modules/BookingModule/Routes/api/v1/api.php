...
Route::group(['prefix' => 'customer', 'as' => 'customer.', 'namespace' => 'Api\V1\Customer', 'middleware' => ['auth:api']], function () {
    Route::group(['prefix'=>'booking','as'=>'booking.'],function (){
        ...
        Route::put('mark-completed/{booking_id}', 'BookingController@mark_completed');
        Route::put('mark_completed_by_renter/{booking_id}', 'BookingController@mark_completed_by_renter');
        Route::put('mark_completed_by_mstoo/{booking_id}', 'BookingController@mark_completed_by_mstoo');

        // custom route by naresh to reshedule booking by customer
        Route::put('schedule-update/{booking_id}', 'BookingController@schedule_update');
        Route::get('request/get', 'BookingController@bookingrequest');
...
Route::group(['prefix' => 'provider', 'as' => 'provider.', 'namespace' => 'Api\V1\Provider', 'middleware' => ['auth:api']], function () {
    Route::group(['prefix'=>'booking','as'=>'booking.'],function (){
        ...
        Route::put('request-reject/{booking_id}', 'BookingController@request_reject');
        Route::put('status-update/{booking_id}', 'BookingController@status_update');
        Route::put('schedule-update/{booking_id}', 'BookingController@schedule_update');
