...
trait BookingTrait
{
    protected function place_booking_request($user_id, $request, $transaction_id)
    {
        ...
        foreach ($cart_data->pluck('sub_category_id')->unique() as $sub_category) {
            ...
            DB::transaction(function () use ($sub_category, $booking, $transaction_id, $request, $cart_data, $user_id) {
                ...
                $ser = DB::table('services')->where('id',$datum['service_id'])->first();

                $booking->provider_id = $ser->added_by;


                // Create order in BORZO API

                // $bookingservice = DB::table('user_addresses')->where('id',$request->service_address_id)->first();
                ...
                // $response = Http::withBody(json_encode($order_data), 'application/json')
                // ->withOptions([
                //     'headers' => [
                ...
                $booking->booking_status = isset($booking->provider_id) ? 'pending' : 'pending';
                ...
                if (isset($booking->provider_id)) {
                    ...
                    Log::info($booking->provider_id);

                    // new code for notification
                    $get_user = DB::table('users')->where('id', $booking->provider_id)->first();
                    $fcm_token = $get_user->fcm_token ?? null;
    ...
    private function update_admin_commission($booking, float $booking_amount, $provider_id)
    {
        ...
        //custom percentage added
        $commission_percentage = business_config('default_commission', 'business_information')->live_values;
    ...
    protected function place_booking_request_borzo($servicename, $servicelocation, $service_contact_info, $booking_address,$booking_contact_no)
    {
                ...
                $order_data = [
                    ...
                    "points" => [
                        [
                            ...
                            "contact_person" => ["phone" => ".$service_contact_info."],
                        ],
                        [
                            "address" => ".$booking_address.",
                            "contact_person" => ["phone" => ".$booking_contact_no."],
                        ],
                    ],
                ];
