...
class BookingController extends Controller
{
    ...
    public function bookingrequest(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            'limit' => 'required|numeric|min:1|max:200',
            'offset' => 'required|numeric|min:1|max:100000',
        ...
        $bookings = $this->booking->with(['customer'])->where(['provider_id' => $request->user()->id])
            ->when($request->has('string'), function ($query) use ($request) {
                $keys = explode(' ', base64_decode($request['string']));
                foreach ($keys as $key) {
                    $query->orWhere('id', 'LIKE', '%' . $key . '%');
    ...
    public function bookingrequestshow(Request $request, string $id): JsonResponse
    {
        $booking = $this->booking->where(['provider_id' => $request->user()->id])->with([
            'detail.service', 'schedule_histories.user', 'status_histories.user', 'service_address', 'customer', 'provider', 'zone', 'serviceman.user'
        ])->where(['id' => $id])->first();
        if (isset($booking)) {
            return response()->json(response_formatter(DEFAULT_200, $booking), 200);
    ...
    public function mark_completed(Request $request, string $booking_id): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            'booking_status' => 'required|in:completed',
        ]);
        ...
        if (isset($booking)) {
            $booking->user_status = $request['booking_status'];
    ...
    public function mark_completed_by_renter(Request $request, string $booking_id): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            'booking_status' => 'required|in:completed',
        ]);
    ...
    public function mark_completed_by_mstoo(Request $request, string $booking_id): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            'booking_status' => 'required|in:completed',
        ]);
        ...
        if (isset($booking)) {
            ...
            $booking_detail = DB::table('booking_details')->where('booking_id',$booking->id)->first();
            $ser = DB::table('services')->where('id',$booking_detail->service_id)->first();
            // Create order in BORZO API

            $bookingservice = DB::table('user_addresses')->where('id',$booking->service_address_id)->first();

            // $this->place_booking_request_borzo($ser->name, $ser->location, $ser->contact_info,$bookingservice->address,$bookingservice->contact_person_number);

            Log::info("service location");
            Log::info($ser->location);
            Log::info("booking location");
            Log::info($bookingservice->address);
                
            Log::info("service address id");
            Log::info($request->service_address_id);

            
            Log::info("booking service phone ");
            Log::info($bookingservice->contact_person_number);

                $order_data = [
                    "matter" => ".$ser->name.",
                    "points" => [
                        [
                            "address" => ".$ser->location.",
                            "contact_person" => ["phone" => ".$ser->contact_info."],
                        ],
                        [
                            "address" => ".$bookingservice->address.",
                            "contact_person" => ["phone" => ".$bookingservice->contact_person_number."],
                        ],
                    ],
                ];

                 Http::withBody(json_encode($order_data), 'application/json')
                ->withOptions([
                    'headers' => [
                        'X-DV-Auth-Token' => '02B3F68BA1160341574B434A4A5D3116A2EE1C4F',
                    ],
                ])
                ->post('https://robotapitest-in.borzodelivery.com/api/business/1.4/create-order');
            // closed
    ...
    // custom function by naresh to reshedule booking by user
    public function schedule_update(Request $request, string $booking_id): JsonResponse
    {
        ...
        if (isset($booking)) {
            ...
            // $booking_schedule_history = DB::table('booking_schedule_histories');
            ...
            DB::table('booking_schedule_histories')->insert(
