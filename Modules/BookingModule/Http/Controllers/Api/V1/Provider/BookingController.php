...
class BookingController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $bookings = $this->booking->with(['customer'])
            ...
            ->when($request['booking_status'] == 'all', function ($query) use ($request) {
                ...
                $query->ofBookingStatus($request['booking_status'])
                    ->whereIn('sub_category_id', $this->subscribed_sub_categories)
                    ->where('zone_id', $request->user()->zone_id)
                    ->where('provider_id', $request->user()->id);
            })->when($request->has('zone_ids'), function ($query) use ($request) {
                $query->whereIn('zone_id', $request['zone_ids']);
            })->when($request->has('from_date') && $request->has('to_date'), function ($query) use ($request) {
    ...
    public function request_accept(Request $request, string $booking_id): JsonResponse
    {
        ...
        // $booking = $this->booking->where('id', $booking_id)->where('provider_id',$request->user()->id)->first();
        $booking = $this->booking->where('id', $booking_id)->first();
        if (isset($booking)) {
            $booking->provider_id = $request->user()->id;
            ...
            DB::transaction(function () use ($booking_status_history, $booking) {
                ...
                $get_user = DB::table('users')->where('id', $booking->customer_id)->first();
                ...
                if(!is_null($fcm_token)) {
                    ...
                    device_notification($fcm_token, translate('Booking Request Accepted'), translate('Your booking has been accepted'), null, $booking->id, 'booking');
                }
    ...
    // custom code to reject booking by renter
    public function request_reject(Request $request, string $booking_id): JsonResponse
    {
        // $booking = $this->booking->where('id', $booking_id)->whereNull('provider_id')->first();
        $booking = $this->booking->where('id', $booking_id)->where('provider_id',$request->user()->id)->first();
        if (isset($booking)) {
            ...
            $booking_status_history->booking_id = $booking_id;
            $booking_status_history->changed_by = $request->user()->id;
            $booking_status_history->booking_status = 'canceled';
            ...
            if(!is_null($fcm_token)) {
                ...
                device_notification($fcm_token, translate('Booking Request Rejected'), translate('Your booking has been rejected'), null, $booking->id, 'booking');
            }
