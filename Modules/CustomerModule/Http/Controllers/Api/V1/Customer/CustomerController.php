...
class CustomerController extends Controller
{
    ...
    public function otp_verification(Request $request): JsonResponse
    {
        ...
        $data = DB::table('password_resets')
            ->where('phone', "+91".$request['phone_or_email'])
    ...
    public function update_profile(Request $request): JsonResponse
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            // 'document' => 'image|mimes:jpeg,jpg,png,gif,pdf|max:10000',
            // 'document' => 'required|file|mimes:jpeg,jpg,png,pdf|max:2048', // Adjust max file size as needed
        ...
        if ($request->has('document')) {
            $customer->document_status = "pending";
            $file_extension = $request->file('document')->extension();
            $customer->document = file_uploader('user/document/', $file_extension, $request->file('document'), $customer->document);

            // $customer->document = file_uploader('user/document/', 'png', $request->file('document'), $customer->document);
        }
