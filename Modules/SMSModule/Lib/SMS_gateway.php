...
class SMS_gateway
{
    public static function send($receiver, $otp, $signatureid): string
    {
    ...
    public static function custom_sms($receiver, $otp, $signatureid): string
    {
        $config = self::get_settings('releans');
        $response = 'error';
        if (isset($config) && $config['status'] == 1) {
            ...
            // https://japi.instaalerts.zone/httpapi/QueryStringReceiver?ver=1.0&key=iQLz6F0g9jPKFUvzFlk2wQ==&encrpt=0&dest=919877512839&send=onetex&dlt_entity_id=1701161475967549249&text=your%20otp%20for%20login%20to%20onetext%20account%20is%20%7b%23var%23%7d
            try {
            Log::info($receiver);

            $number = str_replace("+","",$receiver);

            $smssent =    Http::get('https://japi.instaalerts.zone/httpapi/QueryStringReceiver?ver=1.0&key=iQLz6F0g9jPKFUvzFlk2wQ==&encrpt=0&dest='.$number.'&send=onetex&dlt_entity_id=1701161475967549249&text=your%20otp%20for%20login%20to%20onetext%20account%20is%20'.$otp.'%20'.urlencode($signatureid).'');
