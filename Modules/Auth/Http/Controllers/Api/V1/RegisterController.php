...
class RegisterController extends Controller
{
    ...
    public function customer_register(Request $request): JsonResponse
    {
        ...
        SMS_gateway::send($request->phone, $token);
    ...
    public function provider_register(Request $request): JsonResponse
    {
        ...
        $provider = $this->provider;
        $provider->company_name = $request->account_first_name;
        $provider->company_phone = $request->account_last_name;
        ...
        $provider->contact_person_name = $request->account_first_name;
        $provider->contact_person_phone = $request->contact_person_phone;
        $provider->contact_person_email = $request->account_email;
        $provider->is_approved = 2;
        $provider->is_active = 0;

        $provider->renter_type = $request->renter_type;


        $zone = DB::table('zones')->first();

        if($zone){
        $provider->zone_id = $zone->id;
        }


        // $provider->zone_id = $request['zone_id'];
        // $provider->coordinates = ['latitude' => $request['latitude'], 'longitude' => $request['longitude']];
        $provider->coordinates = ['latitude' => '30.7333148', 'longitude' => '76.7794179'];
        ...
        if (isset($services)) {
            foreach ($services as $service) {
                ...
                // echo "<br>";
                // echo " category_id :".$service->parent_id;
                // echo "<br>";
                // echo " sub_category_id :".$service->id;
                // echo "<br>";
                DB::table('subscribed_services')->insert([
                    'id' => Str::uuid(),
                    'provider_id' => $provider->id,
                    'category_id' => $service->parent_id,
